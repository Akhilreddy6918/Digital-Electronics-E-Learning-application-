...
export const Dashboard: React.FC<DashboardProps> = ({
  ...
  const [demoGate, setDemoGate] = useState<SupportedGate>('AND');

  const computeDemoOutput = (gate: SupportedGate, a: number, b: number): number => {
    switch (gate) {
      case 'AND': return a & b;
      case 'OR': return a | b;
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            ...
            <div className="lg:col-span-5">
              <div className="bg-slate-950 border border-slate-800 rounded-2xl p-5 shadow-2xl relative">
                ...
                {/* Gate Selector Pills (All 8 Gates) */}
                <div className="flex items-center gap-1.5 mb-3 overflow-x-auto pb-1 text-xs">
                  {(['AND', 'OR', 'NOT', 'BUFFER', 'NAND', 'NOR', 'XOR', 'XNOR'] as const).map(gate => (
                    <button
                      key={gate}
                      onClick={() => setDemoGate(gate)}
                      className={`px-2 py-1 rounded-md font-mono text-[11px] font-bold transition-colors cursor-pointer whitespace-nowrap ${
                        demoGate === gate
                ...
                {/* Gate Visual Canvas with Authentic IEEE Symbol */}
                <div className="bg-slate-900/95 rounded-xl p-3 border border-slate-800 flex flex-col items-center gap-3">
                  {/* The Authentic Gate Diagram matching logic gates.jpg */}
                  ...
                  {/* Interactive Input Toggle Buttons & Output LED */}
                  <div className="w-full flex items-center justify-between gap-2 pt-2 border-t border-slate-800 text-xs">
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-lg font-mono font-bold transition-colors border cursor-pointer ${
                          demoInputA === 1
                    ...
                    <div className="flex items-center gap-2 bg-slate-950 px-3 py-1.5 rounded-lg border border-slate-800">
                      <span className="text-[10px] text-slate-400 font-mono">OUTPUT:</span>
                      <span className={`font-mono font-bold text-sm ${demoOutput === 1 ? 'text-emerald-400' : 'text-slate-500'}`}>
                        {demoOutput}
                      </span>
                      <div
                        className={`w-3 h-3 rounded-full border ${
                          demoOutput === 1 ? 'bg-emerald-500 border-emerald-300' : 'bg-slate-700 border-slate-600'
                        }`}
                      />
                    </div>
                  </div>
                ...
                <div className="mt-3 flex items-center justify-between text-xs text-slate-400">
                  <span>Toggle inputs to see the authentic gate symbol update live.</span>
                  <button
                    onClick={() => onOpenCircuit(`gate-${demoGate.toLowerCase()}`)}
